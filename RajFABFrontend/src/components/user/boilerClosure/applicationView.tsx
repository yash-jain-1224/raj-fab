import { boilerClosureInfo } from "@/hooks/api/useBoilers";
...
export default function BoilerClosureDetails({
  formId,
}: {
  formId: string;
}) {
  const { data, isLoading } = boilerClosureInfo(formId);
  ...
  const closureDetails = {
    closureType: appData.closureDetails?.closureType || "",
    closureDate: appData.closureDetails?.closureDate ? new Date(appData.closureDetails.closureDate).toLocaleDateString() : "",
    toStateName: appData.closureDetails?.toStateName || "",
    reasons: appData.closureDetails?.reasons || "",
    remarks: appData.closureDetails?.remarks || "",
  };
  ...
  const documents = {
    ...
    closureReport: appData.closureDetails?.closureReportPath || "",
  };
