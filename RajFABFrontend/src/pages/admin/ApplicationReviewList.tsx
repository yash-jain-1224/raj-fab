...
export default function ApplicationReviewList() {
  ...
  const { approvalRequests: applications, isLoading } = useAppApprovalRequest();
  ...
  const handleRowClick = (app: any) => {
    navigate(`/admin/applications/${app.applicationType.replace(/ /g, "_").toLowerCase()}/${app.applicationId}/${app.approvalRequestId}`);
  };
  ...
  const ApplicationTable = ({ applications }: { applications: any[] }) => (
    <div className="rounded-md border">
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>Application Registration No.</TableHead>
            <TableHead>Application Type</TableHead>
            <TableHead>Applicantion Title</TableHead>
          ...
          {applications.length === 0 ? (
            ...
            applications.map((app) => (
              ...
              >
                ...
                <TableCell>
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      navigate(
                        `/admin/applications/${app.applicationType.replace(/ /g, "_").toLowerCase()}/${app.applicationId}/${app.approvalRequestId}`
                      );
                    }}
                  >
                    Review
                  ...
                  {/* {app.status.toLowerCase() === "approved" && (
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(
                          `/admin/generate-factory-est-certificate/${app.applicationId}`
                        );
                      }}
                    >
                ...
                {/* <TableCell>
                  <Badge
                    variant={app.daysPending > 30 ? "destructive" : "secondary"}
                  >
      ...
      <Tabs defaultValue="all" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="forwarded">
            Forwared ({filterApplications("forwarded").length})
          </TabsTrigger>
        ...
        <TabsContent value="forwarded" className="space-y-4">
          <ApplicationTable applications={filterApplications("forwarded")} />
        </TabsContent>
