...
export default function BoilerManufactureDrawingView() {
  const navigate = useNavigate();
  const { applicationId } = useParams();
  const [actionLoading, setActionLoading] = useState(false);

  const { data, isLoading, error } = useQuery({
    queryKey: ["boilerManufactureDrawing", applicationId],
    queryFn: () => certificateFormsApi.getByApplicationIdBoilerManufactureDrawing(applicationId!),
          ...
          <CardHeader className="bg-gradient-to-r from-primary to-primary/90 text-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle>Boiler Manufacture Drawing Application Details</CardTitle>
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <table className="w-full border-collapse">
              <tbody>
                ...
                {appData.generalInformation && (
                  <>
                    <SectionHeader title="General Information" />
                    {Object.entries(appData.generalInformation).map(([k, v]) => (
                      <InfoRow key={k} label={labelize(k)} value={String(v ?? "")} />
                    ))}
                  </>
                )}

                {appData.addressInformation && (
                  <>
                    <SectionHeader title="Address Information" />
                    {Object.entries(appData.addressInformation).map(([k, v]) => (
                      <InfoRow key={k} label={labelize(k)} value={String(v ?? "")} />
                    ))}
                  </>
                )}

                {appData.boilerDrawingDetails && (
                  <>
                    <SectionHeader title="Boiler Drawing Details" />
                    {Object.entries(appData.boilerDrawingDetails).map(([k, v]) => (
                      <InfoRow key={k} label={labelize(k)} value={String(v ?? "")} />
                    ))}
                  </>
                )}
                ...
                {/* Document links */}
                {(appData.boilerDrawing || appData.feedPipelineDrawing || appData.pressurePartCalculation) && (
                  <>
                    <SectionHeader title="Attachments" />
                    {appData.boilerDrawing && (
                      <tr>
                        <td className="bg-muted/40 px-3 py-2 border font-medium w-1/3 text-sm">Boiler Drawing</td>
                        <td className="px-3 py-2 border text-sm">
                          <Button variant="outline" size="sm" onClick={() => window.open(appData.boilerDrawing, "_blank")}>
                            <Download className="h-4 w-4 mr-1" /> View
                          </Button>
                        </td>
                      </tr>
                    )}
                    {appData.feedPipelineDrawing && (
                      <tr>
                        <td className="bg-muted/40 px-3 py-2 border font-medium w-1/3 text-sm">Feed Pipeline Drawing</td>
                        <td className="px-3 py-2 border text-sm">
                          <Button variant="outline" size="sm" onClick={() => window.open(appData.feedPipelineDrawing, "_blank")}>
                            <Download className="h-4 w-4 mr-1" /> View
                          </Button>
                        </td>
                      </tr>
                    )}
                    {appData.pressurePartCalculation && (
                      <tr>
                        <td className="bg-muted/40 px-3 py-2 border font-medium w-1/3 text-sm">Pressure Part Calculation</td>
                        <td className="px-3 py-2 border text-sm">
                          <Button variant="outline" size="sm" onClick={() => window.open(appData.pressurePartCalculation, "_blank")}>
                            <Download className="h-4 w-4 mr-1" /> View
                          </Button>
                        </td>
                      </tr>
