...
import {
  useBoilers,
  getAllBoilerModificationRepairApplications,
} from "@/hooks/api/useBoilers";
import type { PagedResult } from "@/services/api/boilers";
import formatDate from "@/utils/formatDate";
import { useAuth } from "@/utils/AuthProvider";

export default function BoilerModificationRepairList() {
      ...
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="mt-4 text-muted-foreground">
            Loading boiler modification or repair applications...
          </p>
        </div>
      </div>
    );
          ...
          <div className="flex gap-2 justify-end">
            {app.status?.toLowerCase()?.includes("returned") && (
              <Button
                onClick={() =>
                  navigate(
                    `/user/boilerNew-services/modificationRepair/${encodeURIComponent(app.applicationId)}`,
                    { state: { mode: "update" } },
                  )
                }
                size="sm"
            ...
            <Button
              onClick={() =>
                navigate(
                  `/user/applicationView/boiler_modification/${encodeURIComponent(app.applicationId)}`,
                  { state: { backTo: "/user/boilerNew-services/modificationRepair/list" } },
                )
              }
              size="sm"
              variant="outline"
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight flex items-center gap-3">
            ...
            Boiler Modification or Repair Applications
          </h1>
          <p className="text-muted-foreground mt-2">
            View and manage all your boiler modification or repair applications
          </p>
        </div>
        <div className="flex gap-2">
          <Button
            onClick={() =>
              navigate("/user/boilerNew-services/modificationRepair/create")
            }
            className="gap-2"
          >
            <Plus className="h-4 w-4" />
            New Boiler Modification or Repair
          </Button>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <DataTable
            ...
            data={applications}
            rowKey="id"
            emptyMessage="You haven't submitted any boiler modification or repair application yet."
          />
        </CardContent>
      </Card>
    </div>
