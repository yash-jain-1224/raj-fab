...
import { useWelderApplicationByNumber } from "@/hooks/api/useWelder";
...
export default function WelderView() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useWelderApplicationByNumber(applicationId || "");
  ...
  useEffect(() => {
    if (error) {
      console.error("Failed to fetch welder application:", error);
    }
  }, [error]);
        ...
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4">
            <p className="text-muted-foreground">
              The requested welder application could not be loaded. It may not exist or an error occurred.
            </p>
  ...
  const welderInfo = application.welderDetail || {};
  const employerInfo = application.employerDetail || {};
  ...
  const welderDocuments: { label: string; url: string | null | undefined }[] = [
    { label: "Photo", url: appData.welderDetail?.photo },
    { label: "Thumb", url: appData.welderDetail?.thumb },
    { label: "Welder Signature", url: appData.welderDetail?.welderSign },
    { label: "Employer Signature", url: appData.welderDetail?.employerSign },
  ];
        ...
        <Card>
          <CardHeader className="bg-gradient-to-r from-primary to-primary/80 text-white flex flex-row items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle>Welder Application Details</CardTitle>
          ...
          <CardContent className="p-6">
            ...
            <div className="bg-white border text-sm">
              <table className="w-full border-collapse">
                <tbody>
                  <PreviewSection title="Employer Details" data={employerInfo} />
                  <PreviewSection title="Welder Information" data={welderInfo} />
                  <PreviewSection title="Experience" data={experience} />
                  <PreviewSection title="Kind of Test" data={kindOfTest} />
                  <PreviewSection title="Qualification Details" data={qualification} />
                  {/* Documents */}
                  <tr>
