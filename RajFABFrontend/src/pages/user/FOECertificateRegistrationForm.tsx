...
const [foeErrors,setFoeErrors]=useState<ValidationErrors>({})
...
const validateCurrentStep=():boolean=>{
  ...
  if(step===4){
    const errs=validateRequired(foe,["state","foeNo","date"])
    setFoeErrors(errs)
...
<Field label="State" required error={foeErrors.state}>
<Input value={foe.state} onChange={(e)=>updateFoe("state",e.target.value)} className={foeErrors.state?"border-destructive":""}/>
</Field>
<Field label="FOE No" required error={foeErrors.foeNo}>
<Input value={foe.foeNo} onChange={(e)=>updateFoe("foeNo",e.target.value)} className={foeErrors.foeNo?"border-destructive":""}/>
</Field>
<Field label="Date" required error={foeErrors.date}>
<Input type="date" value={foe.date} onChange={(e)=>updateFoe("date",e.target.value)} className={foeErrors.date?"border-destructive":""}/>
</Field>
<Field label="Certificate">
<DocumentUploader label="" value={foe.certificate} onChange={(url)=>updateFoe("certificate",url)}/>
</Field>
