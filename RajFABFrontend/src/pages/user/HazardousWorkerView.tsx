...
export default function HazardousWorkerView() {
  const navigate = useNavigate();
  const { applicationId } = useParams();
  const [actionLoading, setActionLoading] = useState(false);

  const { data, isLoading, error } = useQuery({
    queryKey: ["hazardousWorker", applicationId],
    queryFn: () => certificateFormsApi.getByApplicationIdHazardousWorker(applicationId!),
  ...
  const work = appData.work || {};
  const medical = appData.medical || {};
          ...
          <CardHeader className="bg-gradient-to-r from-primary to-primary/90 text-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle>Hazardous Waste Worker Application Details</CardTitle>
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <table className="w-full border-collapse">
              <tbody>
                ...
                <InfoRow label="Worker Name" value={personal.workerName} />
                <InfoRow label="Father Name" value={personal.fatherName} />
                <InfoRow label="Date of Birth" value={personal.dob} />
                <InfoRow label="Gender" value={personal.gender} />
                <InfoRow label="Mobile" value={personal.mobile} />
                <InfoRow label="State" value={personal.state} />
                <InfoRow label="District" value={personal.district} />
                <InfoRow label="Address" value={personal.address} />

                <SectionHeader title="Identity Details" />
                <InfoRow label="Aadhaar No" value={identity.aadhaarNo} />
                <InfoRow label="BPL Card" value={identity.bpl} />
                {identity.bpl === "Yes" && <InfoRow label="BPL No" value={identity.bplNo} />}
                <InfoRow label="Bhamashah Card" value={identity.bhamashah} />

                <SectionHeader title="Work Details" />
                <InfoRow label="Service Type" value={work.serviceType} />
                <InfoRow label="Joining Date" value={work.joiningDate} />
                <InfoRow label="Safety Training" value={work.safetyTraining} />
                <InfoRow label="PPEs Provided" value={work.ppes} />
                <InfoRow label="Hazardous Category" value={work.hazardousCategory} />

                <SectionHeader title="Medical Examination" />
                <InfoRow label="X-Ray" value={medical.xray} />
                <InfoRow label="PFT" value={medical.pft} />
                <InfoRow label="Blood Test" value={medical.bloodTest} />

                {(documents.aadhaarCard || documents.photo) && (
                  <>
                    <SectionHeader title="Documents" />
                    {documents.aadhaarCard && (
                      <tr>
                        <td className="bg-muted/40 px-3 py-2 border font-medium w-1/3 text-sm">Aadhaar Card</td>
                        <td className="px-3 py-2 border text-sm">
                          <Button variant="outline" size="sm" onClick={() => window.open(documents.aadhaarCard, "_blank")}>
                    ...
                    {documents.photo && (
                      <tr>
                        <td className="bg-muted/40 px-3 py-2 border font-medium w-1/3 text-sm">Photo</td>
                        <td className="px-3 py-2 border text-sm">
                          <Button variant="outline" size="sm" onClick={() => window.open(documents.photo, "_blank")}>
                            <Download className="h-4 w-4 mr-1" /> View
                          </Button>
                        </td>
                      </tr>
